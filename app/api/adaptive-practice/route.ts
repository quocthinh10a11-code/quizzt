...
function deterministicRank(
  candidates: AdaptivePracticeContext["candidates"],
  targetChapterNames: Set<string>
): string[] {
  return [...candidates]
    .sort((a, b) => {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rateLimitResult = await checkAndRecordRateLimit(
      ...
      userId,
      "insight",
      ADAPTIVE_RATE_LIMIT_PER_MINUTE
    );
    ...
    if (attemptError) {
      console.error("[adaptivePractice] Không thể tải attempt:", attemptError.message);
    ...
    if (answersError) {
      console.error("[adaptivePractice] Không thể tải answers:", answersError.message);
    ...
    for (const answer of sourceAnswers) {
      ...
      const question = (sourceQuestions ?? []).find((item) => item.id === answer.question_id);
    ...
    const targetChapterNames = new Set(targetChapterIds.map((id) => chapterMap.get(id)?.name).filter((name): name is string => !!name));
    ...
    if (candidatesError) {
      console.error("[adaptivePractice] Không thể tải candidate questions:", candidatesError.message);
      return NextResponse.json({ error: "Không thể chọn câu hỏi luyện tập." }, { status: 500 });
    }

    const candidates = (candidateRows ?? []) as (QuestionRow & { quizzes?: { chapter_id: number } | { chapter_id: number }[] })[];
    ...
    const { data: historyAnswers, error: historyAnswersError } = historyAttemptIds.length
      ? await supabase
          ...
          .in("attempt_id", historyAttemptIds)
    ...
    const adaptiveCandidates: AdaptivePracticeContext["candidates"] = candidates.map((candidate) => {
      const quizRelation = candidate.quizzes;
      const chapterId = Array.isArray(quizRelation) ? quizRelation[0]?.chapter_id : quizRelation?.chapter_id;
    ...
    if (selectedIds.length < TARGET_COUNT) {
      const fallbackIds = deterministicRank(adaptiveCandidates, targetChapterNames).map(Number);
    ...
    return NextResponse.json({
      ...
      rationale,
      targetChapterNames: [...targetChapterNames],
