  ...
  async function startAdaptivePractice() {
    ...
    try {
      ...
      const ids = Array.isArray(data.questionIds)
        ? [...new Set(data.questionIds.filter((id: unknown): id is number => Number.isInteger(id) && id > 0))].slice(0, 10)
        : [];
      if (ids.length === 0) {
        setAdaptiveError("Chưa tìm được đủ câu hỏi phù hợp. Bạn có thể tiếp tục với các lựa chọn học hiện có.");
        return;
      }
  ...
  return (
    <Card className="p-5 mt-4 text-left">
      ...
      {!insight && !loading && !error && (
        <Button onClick={loadInsight} variant="secondary" className="mt-4 w-full" size="md">Phân tích phiên học</Button>
      )}
      ...
      {error && (
        <div className="mt-4">
          ...
          <Button onClick={loadInsight} variant="secondary" className="mt-3 w-full" size="md" leftIcon={<RefreshCw size={15} />}>Thử lại</Button>
        </div>
      )}
        ...
        <div className="mt-4 space-y-4">
          ...
          {insight.strengths.length > 0 && (
            <div>
              ...
              <ul className="space-y-1 text-sm text-gray-700 dark:text-gray-300 list-disc pl-5">{insight.strengths.map((item, index) => <li key={`strength-${index}`}>{item}</li>)}</ul>
            </div>
          )}
          {insight.focusAreas.length > 0 && (
            <div>
              ...
              <ul className="space-y-1 text-sm text-gray-700 dark:text-gray-300 list-disc pl-5">{insight.focusAreas.map((item, index) => <li key={`focus-${index}`}>{item}</li>)}</ul>
            </div>
          )}
          ...
          <Button onClick={startAdaptivePractice} variant="primary" className="w-full" size="md" disabled={adaptiveLoading} leftIcon={adaptiveLoading ? <Loader2 size={15} className="animate-spin" /> : <Target size={15} />}>
            {adaptiveLoading ? "Đang chọn câu hỏi..." : "Luyện tập cá nhân hóa"}
          </Button>
